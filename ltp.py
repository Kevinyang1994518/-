...
def install_ltp(option,number):
    ...
    command(cmd)
    time.sleep(10)
    cmd = "apt update && apt install  -y autoconf make gcc build-essential git"
    command(cmd)
    #清理已存在的 /opt/ltp目录

    if os.path.exists("/opt/ltp"):
        command("rm -rvf /opt/ltp")
    ...
    if os.path.exists("ltp"):
        ...
        while True:
            print("开始克隆，第%s次克隆" %x)
            abc = os.system("git clone https://gitlabwh.uniontech.com/ut005203/ltp-mirror.git ltp")
            if abc == 0 :
    ...
    if option == "-f":
        ...
        cmd = "./configure"
        command(cmd)
        cmd = "make all -j`nproc`" # use all cpu core to accelerate
        command(cmd)
        cmd = "make install"
        command(cmd)
        #切换目录到/opt/ltp/testscripts/
...
def sed_email(path,ltpname,whiteList,failList,unknownList):
    ...
    #密码
    passwd = "5h6hb9dnobd2PdPs"

    # 收件人
    receivers = [
        "guobin@uniontech.com","zhengwei@uniontech.com"
        ]
    ...
    try:
        smtp = smtplib.SMTP("smtp.exmail.qq.com", timeout=465)
